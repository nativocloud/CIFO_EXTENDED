...
# MODIFIED FOR 30 RUNS OF PROMISING ALGORITHMS
MULTIPROCESSING_RESULTS_DIR = "/home/ubuntu/CIFO_EXTENDED_Project/images_mp/run_30_results_promising"
...
# MODIFIED FOR 30 RUNS
NUM_RUNS = 30 # Parameter for number of runs (e.g., 5, 10, 30)
...
def main():
    ...
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
    # MODIFIED FOR PROMISING ALGORITHMS
    print(f"[{timestamp}] Selected promising algorithms (HC, SA, GA Config 1 & 4) will be run {NUM_RUNS} times each in parallel (where applicable).")
    
    all_results_summary = []
    # MODIFIED: Use more processes if NUM_RUNS is large, but cap at CPU count or a reasonable number like 10-15 for 30 runs
    # to avoid excessive overhead or resource contention if os.cpu_count() is very high.
    # For 30 runs, using up to 10-15 parallel processes is reasonable if CPUs allow.
    # Let's cap at min(NUM_RUNS, os.cpu_count(), 15) to be safe and efficient.
    # If os.cpu_count() is None or very low, it defaults to min(NUM_RUNS, 1) or min(NUM_RUNS, actual_low_cpu_count)
    max_processes = 15 
    ...
    if hc_all_solutions:
        for i, fit_val in enumerate(hc_all_fitness_values):
            if not np.isnan(fit_val) and fit_val < best_hc_fitness_overall:
                # Ensure corresponding solution and history exist
    ...
    sa_params = {
        ...
        "final_temp": 0.1,
        "alpha": 0.99,
        "iterations_per_temp": 50 # This might need adjustment for 30 runs if too slow
    }
    ...
    if sa_all_solutions:
        for i, fit_val in enumerate(sa_all_fitness_values):
            if not np.isnan(fit_val) and fit_val < best_sa_fitness_overall:
                if i < len(sa_all_solutions) and sa_all_solutions[i] is not None and i < len(sa_all_histories):
    ...
    # --- 3. Genetic Algorithms (Promising Configurations Only) --- 
    ga_section_start_time = time.time()
    timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
    print(f"\n[{timestamp}] --- Starting Genetic Algorithms ({NUM_RUNS} runs per promising config in parallel) ---")

    # MODIFIED FOR PROMISING ALGORITHMS
    ga_configurations = {
        "GA_Config_1_SwapConst1PtPreferVTournVarK": {
            "population_size": 50,
            "generations": 100, # Consider increasing for 30 runs if time permits
        ...
        if ga_all_solutions_config:
            for i, fit_val in enumerate(ga_all_fitness_values_config):
                if not np.isnan(fit_val) and fit_val < best_ga_fitness_config:
                    if i < len(ga_all_solutions_config) and ga_all_solutions_config[i] is not None and i < len(ga_all_histories_config):
