...
from src.solution.solution import (  # LeagueHillClimbingSolution and LeagueSASolution are used in main_script.py directly
    LeagueSolution,
)


def generate_population(
    players_data_full, size, num_teams=5, team_size=7, max_budget=750
):
    ...
    if len(population) < size:
        print(
            f"Warning: Could only generate {len(population)}/{size} valid individuals for initial population after {attempts_total} attempts."
        )
    return population
...
def genetic_algorithm(
    ...
    population = generate_population(
        players_data, population_size, num_teams, team_size, max_budget
    )
    if not population:
    ...
    for gen in range(generations):
        ...
        while len(new_population) < population_size:
            ...
            if selection_operator_func == selection_tournament_variable_k:
                parent1 = selection_operator_func(
                    population, players_data, k=tournament_k
                )
                parent2 = selection_operator_func(
                    population, players_data, k=tournament_k
                )
            elif selection_operator_func == selection_boltzmann:
                parent1 = selection_operator_func(
                    population, players_data, temperature=boltzmann_temp, k=1
                )  # Assuming k=1 for single selection
                parent2 = selection_operator_func(
                    population, players_data, temperature=boltzmann_temp, k=1
                )
            ...
            if child.is_valid():  # MODIFIED: Vectorized call
            ...
            ):  # If child is invalid, try to fill with a new valid one
                ...
                valid_fillers = generate_population(
                    players_data, 1, num_teams, team_size, max_budget
                )
                if valid_fillers:
        ...
        if not new_population:  # If, after all attempts, new_population is empty
            if verbose:
                print(
                    f"Warning: Generation {gen+1} resulted in an empty new population. Stopping GA."
                )
            break
        ...
        if current_best_in_pop.fitness() < best_solution.fitness():
            best_solution = deepcopy(
                current_best_in_pop
            )  # Use deepcopy for solution objects
    ...
    while temp > final_temp:
        for _ in range(iterations_per_temp):
            ...
            if verbose and iteration_count % (iterations_per_temp * 10) == 0:
                print(
                    f"Iter: {iteration_count}, Temp: {temp:.2f}, Current Fitness: {current_fitness:.4f}, Best Fitness: {best_fitness:.4f}"
                )
