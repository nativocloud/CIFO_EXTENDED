...
def mutate_team_shift(solution, players_data):
    new_assign = solution.assignment[:]
    shift = (
        random.randint(1, solution.num_teams - 1) if solution.num_teams > 1 else 0
    )  # Avoid modulo by zero or no shift if 1 team
...
def mutate_shuffle_team(solution, players_data):
    ...
    if solution.num_teams == 0:
        ...
        return LeagueSolution(
            players_data,
            assignment=deepcopy(new_assign),
            num_teams=solution.num_teams,
            team_size=solution.team_size,
            max_budget=solution.max_budget,
        )
    ...
    player_indices_in_chosen_team = [
        i
        for i, team_id_player in enumerate(new_assign)
        if team_id_player == chosen_team_id_to_shuffle
    ]
    ...
    if player_indices_in_chosen_team:
        ...
        new_assign[player_to_reassign_idx] = (
            new_team_for_player  # This is a simple reassignment, likely to break validity.
        )
    ...
    if len(new_assign) > 0:
        ...
        new_team_id = (
            random.randrange(solution.num_teams) if solution.num_teams > 0 else 0
        )
...
def mutate_swap_constrained(solution, players_data, max_attempts=10):
    ...
    for _attempt in range(max_attempts):
        ...
        new_assignment[idx1], new_assignment[idx2] = (
            new_assignment[idx2],
            new_assignment[idx1],
        )
        mutated_solution = LeagueSolution(
            players_data,
...
def mutate_targeted_player_exchange(solution, players_data, max_attempts=20):
    original_assignment = deepcopy(solution.assignment)
    # Removed unused variable: num_players = len(solution.assignment)
    num_teams = solution.num_teams
    ...
    for _attempt in range(max_attempts):
        ...
        players_in_team1_indices = [
            i for i, t_id in enumerate(new_assignment) if t_id == team_idx1
        ]
        players_in_team2_indices = [
            i for i, t_id in enumerate(new_assignment) if t_id == team_idx2
        ]
...
def mutate_shuffle_within_team_constrained(solution, players_data, max_attempts=20):
    ...
    for _attempt in range(max_attempts):
        ...
        if (
            num_teams < 2
        ):  # Need at least two teams for a meaningful exchange between teams
            return LeagueSolution(
                players_data,
                assignment=deepcopy(original_assignment),
                num_teams=solution.num_teams,
        ...
        player_indices_in_chosen_team = [
            i for i, t_id in enumerate(new_assignment) if t_id == chosen_team_id
        ]
        ...
        player_indices_in_other_team = [
            i for i, t_id in enumerate(new_assignment) if t_id == target_other_team_id
        ]
...
def crossover_one_point(parent1, parent2, players_data):
    ...
    if len_assignment < 2:
        ...
        return LeagueSolution(
            players_data,
            assignment=deepcopy(parent1.assignment),
    ...
    cut = random.randint(
        1, len_assignment - 1
    )  # Ensure cut is not at the very beginning or end for a two-part crossover
    child_assign = np.concatenate(
        (parent1.assignment[:cut], parent2.assignment[cut:])
    )  # MODIFIED: Use np.concatenate for NumPy arrays
    # Refactored to correctly pass players_data as the first argument
    return LeagueSolution(
        players_data,
...
def crossover_one_point_prefer_valid(parent1, parent2, players_data, max_attempts=10):
    ...
    for _attempt in range(max_attempts):
        ...
        child_assignment1 = np.concatenate(
            (parent1.assignment[:cut_point], parent2.assignment[cut_point:])
        )  # MODIFIED: Use np.concatenate for NumPy arrays
        child_solution = LeagueSolution(
            players_data,
            assignment=child_assignment1,
            num_teams=parent1.num_teams,
            team_size=parent1.team_size,
            max_budget=parent1.max_budget,
        )
        ...
        last_child_solution = (
            child_solution  # Keep the last generated (possibly invalid) child
        )
...
def selection_ranking(population, players_data):
    ...
    sorted_pop = sorted(
        population, key=lambda s: s.fitness()
    )  # MODIFIED: Vectorized call
    ranks = list(
        range(len(sorted_pop), 0, -1)
    )  # Higher rank for better fitness (lower value)
    total_rank_sum = sum(ranks)

    if (
        total_rank_sum == 0
    ):  # Should only happen if population is empty or all ranks are 0 (not possible with range > 0)
...
def selection_boltzmann(population, players_data, temperature, k=1):
    ...
    fitness_values = np.array(
        [sol.fitness() for sol in population]
    )  # MODIFIED: Vectorized call
    ...
    probabilities_unnormalized[np.isinf(probabilities_unnormalized)] = (
        0  # Should catch exp(inf)
    )
    ...
    if sum_probs == 0 or np.isnan(sum_probs) or np.isinf(sum_probs):
        ...
        return (
            random.choices(population, k=k) if population else (None if k == 1 else [])
        )
