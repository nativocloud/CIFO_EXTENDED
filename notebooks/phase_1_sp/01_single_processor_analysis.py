...
from src.evolution.evolution import (
    genetic_algorithm,
    hill_climbing,
    simulated_annealing,
)
from src.operators.operators import (  # Base Mutations; New/Adapted Mutations; Base Crossovers; New/Adapted Crossovers; Selection Operators
    crossover_one_point,
...
from src.solution.solution import (
...
def print_algorithm_results(
    algo_name, config_name, mean_fitness, std_fitness, mean_exec_time, best_fitness
):
    """Print formatted results for an algorithm run."""
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {algo_name} {config_name} ({NUM_RUNS} runs) processing finished."
    )
...
def save_convergence_plot(history, algo_name, config_name, xlabel="Iteration"):
    ...
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Saved {algo_name} convergence plot to {filepath}"
    )
    plt.close()
...
def compare_convergence_plots(histories_dict, algo_name, xlabel="Iteration"):
    ...
    plt.savefig(filepath)
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Saved {algo_name} comparison plot to {filepath}"
    )
    plt.close()
...
def visualize_statistical_comparison(metrics_data=None):
    ...
    for metric in metrics:
        ...
        for algo in ["HC", "SA", "GA"]:
            for i, config in enumerate(metrics_data[algo]["configs"]):
                if metric_key in metrics_data[algo] and i < len(
                    metrics_data[algo][metric_key]
                ):
    ...
    if (
        "fitness_over_time" in metrics_data["HC"]
        or "fitness_over_time" in metrics_data["SA"]
        or "fitness_over_time" in metrics_data["GA"]
    ):
        ...
        for algo in ["HC", "SA", "GA"]:
            for i, config in enumerate(metrics_data[algo]["configs"]):
                if "fitness_over_time" in metrics_data[algo] and i < len(
                    metrics_data[algo]["fitness_over_time"]
                ):
...
def perform_statistical_tests(metrics_data=None, paired_test=True):
    ...
    print(
        "Null Hypothesis (H₀): The performance metrics are the same across all algorithms"
    )
    ...
    for metric in metrics:
        ...
        if valid_algorithms < 2:
            print(
                f"Not enough algorithms with valid {metric_name} data for statistical comparison."
            )
        ...
        is_paired = (
            paired_test
            and all(len(values) == len(metric_values[0]) for values in metric_values)
            and NUM_RUNS > 1
        )
        ...
        try:
            if valid_algorithms == 2:
                ...
                if p_value < 0.05:
                    print(
                        f"Significant difference detected (p < 0.05) between {algorithm_names[0]} and {algorithm_names[1]}"
                    )
                    ...
                    if lower_is_better:
                        better_idx = (
                            0
                            if np.mean(metric_values[0]) < np.mean(metric_values[1])
                            else 1
                        )
                    else:
                        better_idx = (
                    ...
                    print(
                        f"No significant difference (p >= 0.05) between {algorithm_names[0]} and {algorithm_names[1]}"
                    )
                    results_dict[metric_name] = {
                        "test": test_name,
                        "statistic": float(stat),
                ...
                if is_paired and NUM_RUNS > 1:
                    ...
                    try:
                        ...
                        if p_value < 0.05:
                            print(
                                "Significant difference detected (p < 0.05) among algorithms"
                            )
                            ...
                            try:
                                ...
                                posthoc_filepath = os.path.join(
                                    DATA_DIR_RESULTS,
                                    f"nemenyi_posthoc_{metric_key}.csv",
                                )
                                posthoc_df.to_csv(posthoc_filepath)
                                print(
                                    f"Post-hoc test results saved to {posthoc_filepath}"
                                )
                                    ...
                                    for j in range(i + 1, len(algorithm_names)):
                                        if posthoc[i, j] < 0.05:
                                            if lower_is_better:
                                                better = (
                                                    i
                                                    if np.mean(metric_values[i])
                                                    < np.mean(metric_values[j])
                                                    else j
                                                )
                                            else:
                                                better = (
                                            ...
                                            significant_pairs.append(
                                                (
                                                    algorithm_names[better],
                                                    algorithm_names[worse],
                                                )
                                            )
                                ...
                                if significant_pairs:
                                    ...
                                    for better, worse in significant_pairs:
                                        print(
                                            f"  {better} performs better than {worse}"
                                        )
                                else:
                                    print(
                                ...
                                print(
                                    "scikit-posthocs package not available for Nemenyi test"
                                )
                                print("Install with: pip install scikit-posthocs")
                        else:
                            print(
                                "No significant difference (p >= 0.05) among algorithms"
                            )
                    ...
                    if p_value < 0.05:
                        ...
                        try:
                            ...
                            posthoc = posthoc_dunn(
                                [values for values in metric_values],
                                p_adjust="bonferroni",
                            )
                            print(
                            ...
                            # Create DataFrame with algorithm names
                            posthoc_df = pd.DataFrame(
                                posthoc, index=algorithm_names, columns=algorithm_names
                            )
                            print(posthoc_df)

                            # Save post-hoc results
                            posthoc_filepath = os.path.join(
                                DATA_DIR_RESULTS, f"dunn_posthoc_{metric_key}.csv"
                            )
                            posthoc_df.to_csv(posthoc_filepath)
                            ...
                            print(
                                "scikit-posthocs package not available for Dunn's test"
                            )
                            print("Install with: pip install scikit-posthocs")
                    else:
            ...
            print(
                "For meaningful statistical analysis, increase NUM_RUNS (recommended: 30+)"
            )
    ...
    results_filepath = os.path.join(
        DATA_DIR_RESULTS,
        f"statistical_comparison_{'paired' if paired_test else 'unpaired'}.json",
    )
...
def run_hill_climbing(num_runs=NUM_RUNS, verbose=False):
    ...
    for i in range(num_runs):
        ...
        max_retry_hc = 5
        while (
            not initial_hc_solution_run.is_valid() and retry_attempts_hc < max_retry_hc
        ):
            if verbose:
                print(
                    f"    [{time.strftime('%Y-%m-%d %H:%M:%S')}] HC Run {i+1}: Initial solution invalid, retrying generation ({retry_attempts_hc+1})..."
                )
            initial_hc_solution_run = LeagueHillClimbingSolution(
                players_data,
                num_teams=NUM_TEAMS,
        ...
        if not initial_hc_solution_run.is_valid():
            print(
                f"  [{time.strftime('%Y-%m-%d %H:%M:%S')}] HC Run {i+1} failed to create a valid initial solution after {max_retry_hc} retries. Skipping run."
            )
            hc_all_fitness_values.append(float("nan"))
            hc_all_exec_times.append(float("nan"))
            continue

        hc_solution_obj_run, hc_fitness_val_run, hc_history_convergence_run = (
            hill_climbing(
        ...
        if hc_solution_obj_run:
            ...
            print(
                f"  [{time.strftime('%Y-%m-%d %H:%M:%S')}] HC Run {i+1} did not find a valid solution during search."
            )
            hc_all_fitness_values.append(float("nan"))
            hc_all_exec_times.append(float("nan"))

    hc_mean_fitness = (
        np.nanmean(hc_all_fitness_values) if hc_all_fitness_values else float("nan")
    )
    hc_std_fitness = (
        np.nanstd(hc_all_fitness_values) if hc_all_fitness_values else float("nan")
    )
    hc_mean_exec_time = (
        np.nanmean(hc_all_exec_times) if hc_all_exec_times else float("nan")
    )
    ...
    if best_hc_solution_overall:
        print_algorithm_results(
            ...
            hc_std_fitness,
            hc_mean_exec_time,
            best_hc_fitness_overall,
        )

        # Save convergence plot
        save_convergence_plot(
            best_hc_history_overall, "Hill Climbing", config_name, "Improvement Step"
        )
        ...
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Hill Climbing did not find any valid solution across all runs."
        )
        result_summary = {
            "Algorithm": f"Hill Climbing - {config_name}",
            "Mean Fitness": hc_mean_fitness,
    ...
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Hill Climbing section took {hc_section_end_time - hc_section_start_time:.2f} seconds."
    )
...
def run_simulated_annealing(
    ...
    print(
        f"  Configuration: {config_name} (Initial Temp: {initial_temp}, Final Temp: {final_temp}, Alpha: {alpha}, Iterations/Temp: {iterations_per_temp})"
    )
    ...
    for i in range(num_runs):
        ...
        max_retry_sa = 5
        while (
            not initial_sa_solution_run.is_valid() and retry_attempts_sa < max_retry_sa
        ):
            if verbose:
                print(
                    f"    [{time.strftime('%Y-%m-%d %H:%M:%S')}] SA Run {i+1}: Initial solution invalid, retrying generation ({retry_attempts_sa+1})..."
                )
            initial_sa_solution_run = LeagueSASolution(
                players_data,
                num_teams=NUM_TEAMS,
        ...
        if not initial_sa_solution_run.is_valid():
            print(
                f"  [{time.strftime('%Y-%m-%d %H:%M:%S')}] SA Run {i+1} failed to create a valid initial solution after {max_retry_sa} retries. Skipping run."
            )
            sa_all_fitness_values.append(float("nan"))
            sa_all_exec_times.append(float("nan"))
            continue

        sa_solution_obj_run, sa_fitness_val_run, sa_history_convergence_run = (
            simulated_annealing(
        ...
        if sa_solution_obj_run:
            ...
            print(
                f"  [{time.strftime('%Y-%m-%d %H:%M:%S')}] SA Run {i+1} did not find a valid solution during search."
            )
            sa_all_fitness_values.append(float("nan"))
            sa_all_exec_times.append(float("nan"))

    sa_mean_fitness = (
        np.nanmean(sa_all_fitness_values) if sa_all_fitness_values else float("nan")
    )
    sa_std_fitness = (
        np.nanstd(sa_all_fitness_values) if sa_all_fitness_values else float("nan")
    )
    sa_mean_exec_time = (
        np.nanmean(sa_all_exec_times) if sa_all_exec_times else float("nan")
    )
    ...
    if best_sa_solution_overall:
        print_algorithm_results(
            ...
            sa_std_fitness,
            sa_mean_exec_time,
            best_sa_fitness_overall,
        )

        # Save convergence plot
        save_convergence_plot(
            best_sa_history_overall, "Simulated Annealing", config_name, "Iteration"
        )
        ...
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Simulated Annealing did not find any valid solution across all runs."
        )
        result_summary = {
            "Algorithm": f"Simulated Annealing - {config_name}",
            "Mean Fitness": sa_mean_fitness,
    ...
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Simulated Annealing section took {sa_section_end_time - sa_section_start_time:.2f} seconds."
    )
...
sa_results_standard, sa_best_solution_standard, sa_best_history_standard = (
...
if len(all_results["SA"]["configs"]) > 1:
    ...
    sa_histories_dict = {
        config: all_results["SA"]["histories"][i]
        for i, config in enumerate(all_results["SA"]["configs"])
    }

    # Generate comparative plot
    sa_comparison_plot = compare_convergence_plots(
        sa_histories_dict, "Simulated Annealing"
    )
    ...
    if NUM_RUNS > 1:
        ...
        if len(sa_configs) >= 3:
            try:
                ...
                if p_value < 0.05:
                    print(
                        "There is a statistically significant difference between SA configurations."
                    )
                else:
                    print(
                        "No statistically significant difference detected between SA configurations."
                    )
            except:
                print(
                    "Not enough data for statistical analysis. Run with NUM_RUNS > 1 for statistical tests."
                )
        # T-test if we have exactly 2 configurations
        elif len(sa_configs) == 2:
            try:
                t_stat, p_value = stats.ttest_ind(
                    sa_fitness_values[0], sa_fitness_values[1], equal_var=False
                )
                print(f"Independent t-test: t={t_stat:.4f}, p={p_value:.4f}")

                if p_value < 0.05:
                    print(
                        f"There is a statistically significant difference between {sa_configs[0]} and {sa_configs[1]}."
                    )
                else:
                    print(
                        f"No statistically significant difference detected between {sa_configs[0]} and {sa_configs[1]}."
                    )
            except:
                print(
                    "Not enough data for statistical analysis. Run with NUM_RUNS > 1 for statistical tests."
...
def run_genetic_algorithm(
    ...
    for i in range(num_runs):
        print(
            f"    [{time.strftime('%Y-%m-%d %H:%M:%S')}] GA Run {i+1}/{num_runs} for {config_name}..."
        )
        ...
        if ga_solution_obj_run:
            ...
            print(
                f"    [{time.strftime('%Y-%m-%d %H:%M:%S')}] GA Run {i+1} for {config_name} did not find a valid solution."
            )
            ga_all_fitness_values.append(float("nan"))
            ga_all_exec_times.append(float("nan"))

    ga_mean_fitness = (
        np.nanmean(ga_all_fitness_values) if ga_all_fitness_values else float("nan")
    )
    ga_std_fitness = (
        np.nanstd(ga_all_fitness_values) if ga_all_fitness_values else float("nan")
    )
    ga_mean_exec_time = (
        np.nanmean(ga_all_exec_times) if ga_all_exec_times else float("nan")
    )

    # Print results
    if best_ga_solution_config_overall:
        print(
            f"  [{time.strftime('%Y-%m-%d %H:%M:%S')}] GA Configuration {config_name} ({num_runs} runs) processing finished."
        )
        ...
        print(
            f"    Overall Best GA Fitness for {config_name}: {best_ga_fitness_config_overall:.4f}"
        )
        ...
        print(
            f"  [{time.strftime('%Y-%m-%d %H:%M:%S')}] GA Configuration {config_name} did not find any valid solution across all runs."
        )
        result_summary = {
            "Algorithm": f"Genetic Algorithm - {config_name}",
            "Mean Fitness": ga_mean_fitness,
    ...
    print(
        f"  [{time.strftime('%Y-%m-%d %H:%M:%S')}] GA Configuration {config_name} took {ga_section_end_time - ga_section_start_time:.2f} seconds."
    )

    return (
        result_summary,
        best_ga_solution_config_overall,
        best_ga_history_config_overall,
    )
...
print(
    f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] All Genetic Algorithm configurations took {ga_all_end_time - ga_all_start_time:.2f} seconds."
)
...
if len(all_results["GA"]["configs"]) > 1:
    # Create dictionary of histories for comparison
    ga_histories_dict = {
        config: all_results["GA"]["histories"][i]
        for i, config in enumerate(all_results["GA"]["configs"])
    }

    # Generate comparative plot
    ga_comparison_plot = compare_convergence_plots(
        ga_histories_dict, "Genetic Algorithm", "Generation"
    )
    ...
    if NUM_RUNS > 1:
        ...
        if len(ga_configs) >= 3:
            try:
                ...
                if p_value < 0.05:
                    print(
                        "There is a statistically significant difference between GA configurations."
                    )
                else:
                    print(
                        "No statistically significant difference detected between GA configurations."
                    )
            except:
                print(
                    "Not enough data for statistical analysis. Run with NUM_RUNS > 1 for statistical tests."
        ...
        # T-test if we have exactly 2 configurations
            ...
            try:
                t_stat, p_value = stats.ttest_ind(
                    ga_fitness_values[0], ga_fitness_values[1], equal_var=False
                )
                print(f"Independent t-test: t={t_stat:.4f}, p={p_value:.4f}")

                if p_value < 0.05:
                    print(
                        f"There is a statistically significant difference between {ga_configs[0]} and {ga_configs[1]}."
                    )
                else:
                    print(
                        f"No statistically significant difference detected between {ga_configs[0]} and {ga_configs[1]}."
                    )
            except:
                print(
                    "Not enough data for statistical analysis. Run with NUM_RUNS > 1 for statistical tests."
    ...
    all_results_summary.append(
        {
            ...
            "Mean Fitness": (
                np.nanmean(all_results["HC"]["fitness_values"][i])
                if all_results["HC"]["fitness_values"][i]
                else float("nan")
            ),
            "Std Dev Fitness": (
                np.nanstd(all_results["HC"]["fitness_values"][i])
                if all_results["HC"]["fitness_values"][i]
                else float("nan")
            ),
            "Mean Exec Time (s)": (
                np.nanmean(all_results["HC"]["exec_times"][i])
                if all_results["HC"]["exec_times"][i]
                else float("nan")
            ),
            "Overall Best Fitness": min(
                [x for x in all_results["HC"]["fitness_values"][i] if not np.isnan(x)],
                default=float("nan"),
            ),
            "Mutation Op": "N/A",
            "Crossover Op": "N/A",
    ...
    all_results_summary.append(
        {
            ...
            "Mean Fitness": (
                np.nanmean(all_results["SA"]["fitness_values"][i])
                if all_results["SA"]["fitness_values"][i]
                else float("nan")
            ),
            "Std Dev Fitness": (
                np.nanstd(all_results["SA"]["fitness_values"][i])
                if all_results["SA"]["fitness_values"][i]
                else float("nan")
            ),
            "Mean Exec Time (s)": (
                np.nanmean(all_results["SA"]["exec_times"][i])
                if all_results["SA"]["exec_times"][i]
                else float("nan")
            ),
            "Overall Best Fitness": min(
                [x for x in all_results["SA"]["fitness_values"][i] if not np.isnan(x)],
                default=float("nan"),
            ),
            "Mutation Op": "N/A",
            "Crossover Op": "N/A",
    ...
    all_results_summary.append(
        {
            ...
            "Mean Fitness": (
                np.nanmean(all_results["GA"]["fitness_values"][i])
                if all_results["GA"]["fitness_values"][i]
                else float("nan")
            ),
            "Std Dev Fitness": (
                np.nanstd(all_results["GA"]["fitness_values"][i])
                if all_results["GA"]["fitness_values"][i]
                else float("nan")
            ),
            "Mean Exec Time (s)": (
                np.nanmean(all_results["GA"]["exec_times"][i])
                if all_results["GA"]["exec_times"][i]
                else float("nan")
            ),
            "Overall Best Fitness": min(
                [x for x in all_results["GA"]["fitness_values"][i] if not np.isnan(x)],
                default=float("nan"),
            ),
            "Mutation Op": (
                ga_results[i]["Mutation Op"] if i < len(ga_results) else "N/A"
            ),
            "Crossover Op": (
                ga_results[i]["Crossover Op"] if i < len(ga_results) else "N/A"
            ),
            "Selection Op": (
                ga_results[i]["Selection Op"] if i < len(ga_results) else "N/A"
            ),
        }
    )
...
if (
    all_results["HC"]["histories"]
    and all_results["SA"]["histories"]
    and all_results["GA"]["histories"]
):
...
print(
    summary_df[display_cols].sort_values("Overall Best Fitness").to_string(index=False)
)
...
def main(run_hc=True, run_sa=True, run_ga=True, num_runs=1):
    ...
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Single-Processor Script execution started."
    )

    data_load_start_time = time.time()
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Loading player data...")
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Player data loaded successfully. Total players: {len(players_data)}"
    )
    if players_data:
        print(
            f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] First player data: {players_data[0]}"
        )
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] All algorithms (HC, SA, GA) will be run {NUM_RUNS} times each."
    )
    data_load_end_time = time.time()
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Data loading and setup took {data_load_end_time - data_load_start_time:.2f} seconds."
    )

    # Run Hill Climbing if requested
    if run_hc:
        hc_results, hc_best_solution, hc_best_history = run_hill_climbing(
            num_runs=NUM_RUNS
        )
    ...
    if run_sa:
        sa_results_standard, sa_best_solution_standard, sa_best_history_standard = (
            run_simulated_annealing(num_runs=NUM_RUNS, config_name="Standard")
        )
    ...
    if (
        (run_hc and all_results["HC"]["histories"])
        or (run_sa and all_results["SA"]["histories"])
        or (run_ga and all_results["GA"]["histories"])
    ):
                ...
                all_results_summary.append(
                    {
                        ...
                        "Overall Best Fitness": min(
                            [
                                x
                                for x in all_results["HC"]["fitness_values"][i]
                                if not np.isnan(x)
                            ],
                            default=float("nan"),
                        ),
                ...
                all_results_summary.append(
                    {
                        ...
                        "Overall Best Fitness": min(
                            [
                                x
                                for x in all_results["SA"]["fitness_values"][i]
                                if not np.isnan(x)
                            ],
                            default=float("nan"),
                        ),
                ...
                all_results_summary.append(
                    {
                        ...
                        "Overall Best Fitness": min(
                            [
                                x
                                for x in all_results["GA"]["fitness_values"][i]
                                if not np.isnan(x)
                            ],
                            default=float("nan"),
                        ),
        ...
        if (
            (run_hc and all_results["HC"]["histories"])
            and (run_sa and all_results["SA"]["histories"])
            and (run_ga and all_results["GA"]["histories"])
        ):
            all_algorithms_plot = compare_all_algorithms()
    ...
    print(
        f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Single-Processor Script execution finished. Total time: {script_total_end_time - script_total_start_time:.2f} seconds."
    )
...
def run_statistical_analysis():
    ...
    if comparison_data:
        ...
        comparison_filepath = os.path.join(
            DATA_DIR_RESULTS, "paired_vs_unpaired_comparison.csv"
        )
        ...
        print(
            f"Agreement rate: {agreement_count}/{len(comparison_data)} metrics ({agreement_count/len(comparison_data)*100:.1f}%)"
        )

        # List disagreements
        disagreements = [
            row["Metric"] for row in comparison_data if not row["Agreement"]
        ]
        if disagreements:
            print(
                f"Metrics with disagreement between paired and unpaired tests: {', '.join(disagreements)}"
            )
        else:
