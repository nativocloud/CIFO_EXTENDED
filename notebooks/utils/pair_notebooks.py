...
def get_best_kernel(kernel_name=None):
    ...
    if DEFAULT_KERNEL_NAME in available_kernels:
        ...
        return {
            ...
            "display_name": kernel_info.get("spec", {}).get("display_name", DEFAULT_KERNEL_DISPLAY)
        }
...
def pair_all_notebooks(directory='.', kernel_name=None):
    ...
    print("3. When editing .py files, restart the kernel to see changes in the notebook")
...
if __name__ == "__main__":
    ...
    parser.add_argument('script', nargs='?', help='Path to the Python script (optional)')
